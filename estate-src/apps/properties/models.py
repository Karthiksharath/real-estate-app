...
class PublishManager(models.Manager):
  ...
  def get_queryset(self):
    return super(PublishManager, self).get_queryset().filter(published=True)

  # reusable method to filter out propertirs with published status = True

User = get_user_model()

class PropertyModel(CommonModel):
    ...
    advert_type = models.CharField(
        ...
        default=ADVERT_TYPE_CHOICES.For_Sale,
    )

    property_type = models.CharField(
        verbose_name=_("Property Type"),
        max_length=50,
        choices=PROPERTY_TYPE_CHOICES.choices,
        default=PROPERTY_TYPE_CHOICES.Other,
    )
...
class PropertyViewsModel(CommonModel):

    ip = models.CharField(verbose_name=_("IP Address"), max_length=250)
    property = models.ForeignKey(
        PropertyModel, related_name="property_views", on_delete=models.CASCADE
    )
